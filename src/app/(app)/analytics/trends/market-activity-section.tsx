                    ...
                    {areas.map(area => {
                        ...
                        return (
                            <React.Fragment key={area.id}>
                                <Bar key={`${area.id}_new`} yAxisId="left" dataKey={`${area.id}_new`} name={`${prefix}New`} stackId={area.id} fill={area.color} radius={[0, 0, 0, 0]} />
                                <Bar yAxisId="left" dataKey={`${area.id}_closed`} name={`${prefix}Closed`} stackId={area.id} fill={withOpacity(area.color, 0.35)} radius={[3, 3, 0, 0]} />
                                <Line yAxisId="right" type="monotone" dataKey={`${area.id}_acc`} name={`${prefix}Inventory`} stroke={area.color} strokeWidth={2} strokeDasharray="4 3" dot={false} activeDot={{ r: 4 }} />
                            </React.Fragment>
                        );
                    })}
                </ComposedChart>
