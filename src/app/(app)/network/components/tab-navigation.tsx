...
import { CheckIcon, SearchIcon, SortIcon, StarIcon } from "../icons";
import { type SortBy, usePeople } from "../people-context";
...
export function TabNavigation({ sortBy, reverse, onSortChange, onReverseChange }: TabNavigationProps) {
    ...
    const { people, setSelectedIds, searchQuery, setSearchQuery, refetchPeople, showStarredOnly, setShowStarredOnly, selectedPerson, setSelectedPerson } =
        usePeople();
    ...
    useEffect(() => {
        const handleKeyDown = (e: KeyboardEvent) => {
            ...
            if (target.tagName === "TEXTAREA" || (target.isContentEditable && target !== searchInputRef.current)) {
                return;
            }
            ...
            <div className="flex items-center justify-between">
                    ...
                    {tabs.map((tab) => {
                        ...
                        return (
                            <Link
                                ...
                                className={cn(
                                    "rounded-none border-b-2 px-0 py-1 text-sm font-medium transition-colors",
                                    active
                ...
                <div className="flex items-center gap-3">
                    ...
                    {pathname === "/network" && (
                        ...
                        >
                            <StarIcon
                                className={cn("h-4 w-4 transition-colors", showStarredOnly ? "text-amber-400" : "text-current")}
                        ...
                        <>
                            {isSearchExpanded ? (
                                <div className="relative" data-tour="search-bar">
                                    <SearchIcon className="absolute top-1/2 left-2.5 h-4 w-4 -translate-y-1/2 text-gray-400 dark:text-gray-500" />
                                    <Input
                                        ref={searchInputRef}
                                        type="text"
                                        placeholder="Search"
                                        ...
                                        className="h-8 w-48 border-gray-200 bg-gray-50 pr-8 pl-8 text-sm text-gray-900 placeholder:text-gray-400 dark:border-gray-700 dark:bg-gray-800 dark:text-gray-100 dark:placeholder:text-gray-500"
                                    />
                                    <button
                                        onClick={() => {
                                            setSearchQuery("");
                        ...
                        <>
                            <DropdownMenu>
                                <DropdownMenuTrigger asChild>
                                    <button className="flex items-center gap-1.5 rounded-md px-2 py-1 text-sm text-gray-600 hover:bg-gray-100 hover:text-gray-900 dark:text-gray-400 dark:hover:bg-gray-800 dark:hover:text-gray-100">
                                        <span>{sortBy === "recency" ? "Recency" : sortBy === "alphabetical" ? "Alphabetical" : "Network Strength"}</span>
                                        <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                ...
                                <DropdownMenuContent align="end" className="w-48">
                                    ...
                                    <DropdownMenuItem onClick={() => onReverseChange(!reverse)}>
                                        <SortIcon className={cn("mr-2 h-4 w-4", reverse && "rotate-180")} />
