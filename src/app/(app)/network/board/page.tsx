...
export default function BoardPage() {
    const { people, setPeople, selectedPerson, setSelectedPerson, showStarredOnly } = usePeople();
    ...
    const handleAddPersonToColumn = async (personId: string, columnId: string) => {
        ...
        setKanbanColumns((prevColumns) => prevColumns.map((col) => (col.id === columnId ? { ...col, cards: [...col.cards, newCard] } : col)));
        ...
        try {
            ...
            setKanbanColumns((prevColumns) =>
                prevColumns.map((col) => (col.id === columnId ? { ...col, cards: col.cards.filter((card) => card.id !== newCard.id) } : col)),
            );
        }
    };
    ...
    const handleCardDragEnd = async () => {
        if (draggedCard && draggedOverColumn) {
            const isNewCard = draggedCard.startsWith("card-") && !kanbanColumns.some((col) => col.cards.some((c) => c.id === draggedCard));
            ...
            if (isNewCard) {
                ...
                card = kanbanColumns.flatMap((col) => col.cards).find((c) => c.id === draggedCard) || null;
                ...
                oldColumn = kanbanColumns.find((col) => col.cards.some((c) => c.id === draggedCard)) || null;
            }
                ...
                <div className="flex-1 overflow-x-auto overflow-y-hidden bg-white dark:bg-gray-900">
                    <div className="flex h-full gap-4 p-4">
                        {[...Array(4)].map((_, i) => (
                            <div
                                key={i}
                                className="flex w-64 flex-shrink-0 flex-col overflow-hidden rounded-lg border border-gray-200 bg-gray-50 dark:border-gray-700 dark:bg-gray-800"
                            >
                                ...
                                <div className="flex-1 overflow-x-hidden overflow-y-auto p-2">
                                    <div className="space-y-2">
                                        {[...Array(3)].map((_, j) => (
                                            <div key={j} className="rounded-lg border border-gray-200 bg-white p-3 dark:border-gray-700 dark:bg-gray-900">
