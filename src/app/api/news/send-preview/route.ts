...
async function sendNewsletterInBackground(
    ...
    console.log(
        `Sending newsletter - Email: ${email}, Interests: "${interests}", Counties: ${counties ? counties.join(", ") : "all"}, Cities: ${cities ? cities.join(", ") : "none"}`,
    );

    // Fetch articles from database
    const { nationalArticles, localArticles } = await fetchArticlesForNewsletter(counties, cities, interests, "gemini-2.5-flash-lite");
...
export async function POST(request: NextRequest) {
    try {
        ...
        console.log(
            `Send request - Email: ${email}, Interests: "${interests}", Counties: ${counties ? counties.join(", ") : "all"}, Cities: ${cities ? cities.join(", ") : "none"}`,
        );
        ...
        return NextResponse.json(
            {
                ...
                html:
                    '<div class="text-center p-8"><h2 class="text-2xl font-bold text-green-600 mb-4">✓ Newsletter Sent!</h2><p class="text-gray-600">Check your inbox at ' +
                    email +
                    "</p></div>",
            },
            { status: 200 },
