...
export async function POST(request: NextRequest) {
    try {
        ...
        if (!action || !interests) {
            return NextResponse.json({ error: "Missing required fields: action and interests" }, { status: 400 });
        }
  ...
  "questions": [
    ...
    "What geographic markets are you focused on?"
            ...
            if (!conversation || !Array.isArray(conversation)) {
                return NextResponse.json({ error: "Missing or invalid conversation array" }, { status: 400 });
            }

            const conversationText = conversation.map((turn: { question: string; answer: string }) => `Q: ${turn.question}\nA: ${turn.answer}`).join("\n\n");
