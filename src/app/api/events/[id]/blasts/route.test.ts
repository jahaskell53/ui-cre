...
function setupBlastsQuery(blasts: unknown[], error: unknown = null) {
    const mockOrder = vi.fn().mockResolvedValue({ data: blasts, error });
...
describe("GET /api/events/[id]/blasts — happy path", () => {
    ...
    it("returns blasts for event owner", async () => {
        mockGetUser.mockResolvedValue({ data: { user: { id: "user-1" } }, error: null });
        const blasts = [{ id: "blast-1", event_id: "evt-1", sent_count: 5 }];
        ...
        mockFrom.mockImplementation((table: string) => {
            ...
            if (table === "event_blasts") {
                const { mockSelect } = setupBlastsQuery(blasts);
                return { select: mockSelect };
            }
            return {};
        });
    ...
    it("returns empty array when no blasts", async () => {
        ...
        mockFrom.mockImplementation((table: string) => {
            ...
            if (table === "event_blasts") {
                const { mockSelect } = setupBlastsQuery([]);
                return { select: mockSelect };
            }
            return {};
        });
    ...
    it("returns 500 when blasts fetch fails", async () => {
        mockGetUser.mockResolvedValue({ data: { user: { id: "user-1" } }, error: null });
        ...
        mockFrom.mockImplementation((table: string) => {
            ...
            if (table === "event_blasts") {
                const { mockSelect } = setupBlastsQuery([], { message: "DB error" });
                return { select: mockSelect };
            }
            return {};
        });
