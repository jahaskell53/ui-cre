  ...
  try {
    ...
    if (error) {
      ...
      return NextResponse.redirect(
        new URL('/onboarding/connect-email?error=oauth_failed', request.url)
      );
    }

    if (!code || !state) {
      return NextResponse.redirect(
        new URL('/onboarding/connect-email?error=missing_params', request.url)
      );
    }
    ...
    const redirectTo = stateParts[2] ? decodeURIComponent(stateParts[2]) : '/onboarding/connect-email';
    ...
    return NextResponse.redirect(
      new URL('/onboarding/connect-email?error=callback_failed', request.url)
    );
  }
}
