...
describe('POST /api/meet — auth', () => {
  ...
  it('returns 401 when not authenticated', async () => {
    ...
    const res = await POST(
      makePost({ title: 'Meeting', start_time: '2026-03-01T10:00:00Z', end_time: '2026-03-01T11:00:00Z' })
    )
...
describe('POST /api/meet — happy path', () => {
  ...
  it('returns meet_link on success', async () => {
    ...
    const res = await POST(
      makePost({ title: 'Team Sync', start_time: '2026-03-01T10:00:00Z', end_time: '2026-03-01T11:00:00Z' })
    )
    const body = await res.json()
