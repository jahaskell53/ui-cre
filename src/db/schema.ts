...
export const loopnetListingDetails = pgTable(
    ...
    (table) => [
        ...
        index("idx_loopnet_listing_details_address_city_state_lower").using("btree", sql`lower(address_city)`, sql`lower(address_state)`),
        index("idx_loopnet_listing_details_geom")
...
export const loopnetListingSnapshots = pgTable(
    ...
    (table) => [
        uniqueIndex("loopnet_listing_snapshots_url_run_key").on(table.listingUrl, table.runId),
        index("idx_loopnet_listing_snapshots_run_id").using("btree", table.runId.desc().nullsFirst()),
        index("idx_loopnet_listing_snapshots_scraped_at").using("btree", table.scrapedAt.asc().nullsLast()),
        pgPolicy("Enable read access for all users", { as: "permissive", for: "select", to: ["public"], using: sql`true` }),
    ],
);

/** Historical LoopNet listing rows moved out of `loopnet_listings` (e.g. renumbered run archive). */
export const loopnetListingsArchived = pgTable(
    "loopnet_listings_archived",
    {
        id: uuid().defaultRandom().primaryKey().notNull(),
        listingUrl: text("listing_url").notNull(),
        thumbnailUrl: text("thumbnail_url"),
    ...
    (table) => [
        index("idx_loopnet_listings_archived_listing_url").using("btree", table.listingUrl.asc().nullsLast().op("text_ops")),
        index("idx_loopnet_listings_archived_address_city_state_lower").using("btree", sql`lower(address_city)`, sql`lower(address_state)`),
        uniqueIndex("loopnet_listings_archived_listing_url_run_id_key").on(table.listingUrl, table.runId),
        index("idx_loopnet_listings_archived_geom")
            .using("gist", table.geom.asc().nullsLast().op("gist_geometry_ops_2d"))
            .where(sql`(geom IS NOT NULL)`),
        pgPolicy("Enable read access for all users", { as: "permissive", for: "select", to: ["public"], using: sql`true` }),
    ],
...
/** Historical price snapshots moved out of `loopnet_listing_snapshots` (e.g. renumbered run archive). */
export const loopnetListingSnapshotsArchived = pgTable(
    "loopnet_listing_snapshots_archived",
    {
        id: uuid().defaultRandom().primaryKey().notNull(),
        listingUrl: text("listing_url").notNull(),
        runId: integer("run_id").notNull(),
    ...
    (table) => [
        uniqueIndex("loopnet_listing_snapshots_archived_url_run_key").on(table.listingUrl, table.runId),
        index("idx_loopnet_listing_snapshots_archived_run_id").using("btree", table.runId.desc().nullsFirst()),
        index("idx_loopnet_listing_snapshots_archived_scraped_at").using("btree", table.scrapedAt.asc().nullsLast()),
        pgPolicy("Enable read access for all users", { as: "permissive", for: "select", to: ["public"], using: sql`true` }),
    ],
);
...
export const crexiCompsRecords = pgTable("crexi_comps_records", {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    property_link: text("property_link"),
    ...
    sold_price: doublePrecision("sold_price"),
    lease_signed: text("lease_signed"),
    lease_rate: text("lease_rate"),
    lender: text("lender"),
    loan_amount: doublePrecision("loan_amount"),
    loan_type: text("loan_type"),
    interest_rate: doublePrecision("interest_rate"),
    financing_maturity_date: text("financing_maturity_date"),
    zoning_code: text("zoning_code"),
    opportunity_zone: text("opportunity_zone"),
    water_code: text("water_code"),
    sewer_code: text("sewer_code"),
    building_sqft: doublePrecision("building_sqft"),
    number_of_units: doublePrecision("number_of_units"),
    number_of_addresses: doublePrecision("number_of_addresses"),
    building_count: doublePrecision("building_count"),
    number_of_stories: doublePrecision("number_of_stories"),
    year_built: doublePrecision("year_built"),
    lot_size_acres: doublePrecision("lot_size_acres"),
    lot_size_sqft: doublePrecision("lot_size_sqft"),
    asking_cap_rate: doublePrecision("asking_cap_rate"),
    closing_noi: doublePrecision("closing_noi"),
    closing_cap_rate: doublePrecision("closing_cap_rate"),
    sold_price_per_sqft: doublePrecision("sold_price_per_sqft"),
    sold_price_per_acre: doublePrecision("sold_price_per_acre"),
    occupancy: text("occupancy"),
    lease_type: text("lease_type"),
    lease_commencement: text("lease_commencement"),
    lease_term: text("lease_term"),
    lease_term_remaining: text("lease_term_remaining"),
    lease_expiration_date: text("lease_expiration_date"),
    rent_bumps: text("rent_bumps"),
    lease_options: text("lease_options"),
    tenants: text("tenants"),
    days_on_market: doublePrecision("days_on_market"),
    latitude: doublePrecision("latitude"),
    longitude: doublePrecision("longitude"),
    usps_vacancy: text("usps_vacancy"),
    usps_vacancy_date: text("usps_vacancy_date"),
    parcel_value_type: text("parcel_value_type"),
    improvement_value: doublePrecision("improvement_value"),
    land_value: doublePrecision("land_value"),
    total_parcel_value: doublePrecision("total_parcel_value"),
    tax_year: doublePrecision("tax_year"),
    annual_tax_bill: doublePrecision("annual_tax_bill"),
    pfc_recording_date: text("pfc_recording_date"),
    pfc_indicator: text("pfc_indicator"),
    pfc_document_type: text("pfc_document_type"),
    reo_sale_flag: text("reo_sale_flag"),
    transaction_event_type: text("transaction_event_type"),
    owner_name: text("owner_name"),
    mailing_address: text("mailing_address"),
    mailing_address_unit: text("mailing_address_unit"),
    mailing_address_city: text("mailing_address_city"),
    mailing_address_state: text("mailing_address_state"),
    mailing_address_zip_code: text("mailing_address_zip_code"),
    mailing_address_care_of: text("mailing_address_care_of"),
    created_at: timestamp("created_at", { withTimezone: true }).defaultNow(),
});

export const crexiActiveListings = pgTable("crexi_active_listings", {
    id: bigserial("id", { mode: "number" }).primaryKey(),
    property_link: text("property_link"),
    property_name: text("property_name"),
    property_status: text("property_status"),
    type: text("type"),
    address: text("address"),
    city: text("city"),
    state: text("state"),
    ...
    tenants: text("tenants"),
    lease_term: text("lease_term"),
    remaining_term: text("remaining_term"),
    sqft: doublePrecision("sqft"),
    lot_size: text("lot_size"),
    units: doublePrecision("units"),
    price_per_unit: doublePrecision("price_per_unit"),
    noi: doublePrecision("noi"),
    cap_rate: doublePrecision("cap_rate"),
    asking_price: doublePrecision("asking_price"),
    price_per_sqft: doublePrecision("price_per_sqft"),
    price_per_acre: doublePrecision("price_per_acre"),
    days_on_market: doublePrecision("days_on_market"),
    opportunity_zone: text("opportunity_zone"),
    longitude: doublePrecision("longitude"),
    latitude: doublePrecision("latitude"),
...
export const crexiApiComps = pgTable("crexi_api_comps", {
    ...
    property_price_total: doublePrecision("property_price_total"),
    ...
    occupancy_rate_percent: doublePrecision("occupancy_rate_percent"),
    year_built: integer("year_built"),
    lot_size_sqft: doublePrecision("lot_size_sqft"),
    lot_size_acre: doublePrecision("lot_size_acre"),
    zoning: text("zoning"),
    is_opportunity_zone: boolean("is_opportunity_zone"),
    owner_name: text("owner_name"),
    is_corporate_owner: boolean("is_corporate_owner"),
    is_crexi_source: boolean("is_crexi_source"),
    investment_type: text("investment_type"),
    stories_count: integer("stories_count"),
...
/**
 * Search-index / universal-search record payload. Append-only bronze keyed by
 * `(crexi_id, run_id)` so future scrape runs preserve historical snapshots.
 * The `on delete cascade` from `crexi_api_comps` is intentionally removed so
 * bronze outlives silver.
 */
export const crexiApiCompRawJson = pgTable(
    "crexi_api_comp_raw_json",
    {
        ...
        run_id: bigint("run_id", { mode: "number" })
            .notNull()
            .references(() => crexiScrapeRuns.run_id),
        fetched_at: timestamp("fetched_at", { withTimezone: true }).defaultNow().notNull(),
    },
    (table) => [
        primaryKey({ columns: [table.crexi_id, table.run_id], name: "crexi_api_comp_raw_json_pkey" }),
        index("idx_crexi_api_comp_raw_json_crexi_fetched_at").using(
            "btree",
            table.crexi_id.asc().nullsLast().op("text_ops"),
            table.fetched_at.desc().nullsFirst().op("timestamptz_ops"),
        ),
        index("idx_crexi_api_comp_raw_json_run_id").using("btree", table.run_id.asc().nullsLast().op("int8_ops")),
    ],
);
...
export const crexiApiCompDetailJson = pgTable(
    ...
    (table) => [
        primaryKey({ columns: [table.crexi_id, table.run_id], name: "crexi_api_comp_detail_json_pkey" }),
        index("idx_crexi_api_comp_detail_json_crexi_fetched_at").using(
            "btree",
            table.crexi_id.asc().nullsLast().op("text_ops"),
            table.fetched_at.desc().nullsFirst().op("timestamptz_ops"),
        ),
        index("idx_crexi_api_comp_detail_json_run_id").using("btree", table.run_id.asc().nullsLast().op("int8_ops")),
    ],
);
