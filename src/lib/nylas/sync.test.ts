...
const {
  ...
  mockFrom,
  mockRecalculateNetworkStrength,
} = vi.hoisted(() => ({
 ...
 */
function setupIntegrationSupabase(integration: Record<string, unknown> = {
  id: 'int-1',
  email_address: null,
  first_sync_at: null,
  last_sync_at: null,
 ...
 */
function setupSuccessSupabase(integration: Record<string, unknown> = {
  id: 'int-1',
...
describe('syncAllContacts — successful sync', () => {
  ...
  it('updates integration to active with last_sync_at on success', async () => {
    ...
    expect(mockUpdate).toHaveBeenCalledWith(
      expect.objectContaining({ status: 'active', last_sync_at: expect.any(String), sync_error: null })
    )
  })
  ...
  it('sets first_sync_at when this is the first sync', async () => {
    ...
    expect(mockUpdate).toHaveBeenCalledWith(
      expect.objectContaining({ first_sync_at: expect.any(String) })
    )
  })
  ...
  it('does not overwrite first_sync_at on subsequent syncs', async () => {
    ...
    expect(mockUpdate).toHaveBeenCalledWith(
      expect.not.objectContaining({ first_sync_at: expect.anything() })
    )
  })
