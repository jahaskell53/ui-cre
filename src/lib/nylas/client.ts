import { NYLAS_SYNC_CONFIG, assertNylasConfigured, getNylasClient, getProviderScopes, nylasConfig } from "./config";
import type { Provider } from "./config";
import { getMockCalendarEvents, getMockCalendars, getMockGrant, getMockMessages, isMockMode } from "./mock";
...
export class NylasRateLimitError extends Error {
    ...
    constructor(message: string, statusCode: number, retryAfterMs: number, itemsCollected: number, headers?: Record<string, string>) {
        super(message);
 ...
 */
async function collectPaginatedItems<T>(createAsyncList: () => any, maxItems: number, context: string): Promise<T[]> {
    const items: T[] = [];
    let pageCount = 0;
    let retryCount = 0;
    ...
    while (items.length < maxItems && retryCount <= BACKOFF_CONFIG.maxRetries) {
        try {
            ...
            if (isRetryableError(error) && retryCount < BACKOFF_CONFIG.maxRetries) {
                ...
                console.log(
                    `[${context}] Retryable error (${error?.statusCode}). Waiting ${Math.round(delay / 1000)}s before retry ${retryCount + 1}/${BACKOFF_CONFIG.maxRetries}`,
                );
 ...
 */
export async function getMessages(grantId: string, limit: number = NYLAS_SYNC_CONFIG.emailLimit, receivedAfter?: number): Promise<NylasMessage[]> {
    ...
    if (receivedAfter) {
        ...
        console.log(
            `Fetching up to ${limit} messages received after ${new Date(receivedAfter * 1000).toISOString()} (using limit=${requestLimit} per request)`,
        );
    } else {
    ...
    try {
        const allMessages = await collectPaginatedItems<NylasMessage>(createMessageIterator, limit, "messages");
 ...
 * @throws {NylasRateLimitError} When rate limited after max retries
 */
export async function getCalendarEvents(grantId: string, limit: number = NYLAS_SYNC_CONFIG.calendarLimit, startAfter?: number): Promise<NylasCalendarEvent[]> {
    ...
    for (const calendar of calendars) {
        ...
        try {
            const calendarEvents = await collectPaginatedItems<NylasCalendarEvent>(createEventIterator, calendarLimit, `calendar:${calendar.id}`);
