 ...
 */
async function batchCheckAutomatedEmails(emailInputs: EmailCheckInput[], cache: Map<string, boolean>, trace?: any): Promise<Map<string, boolean>> {
 ...
 */
async function isAutomatedEmail(email: string, message?: NylasMessage, name?: string, cache?: Map<string, boolean>): Promise<boolean> {
...
function isAutomatedEmailFallback(email: string, message?: NylasMessage, name?: string): boolean {
    ...
    if (message?.headers) {
        ...
        if (headers["precedence"]?.toLowerCase() === "bulk" || headers["Precedence"]?.toLowerCase() === "bulk") {
            return true;
        }
    ...
    const disposableDomains = ["mailinator.com", "guerrillamail.com", "10minutemail.com", "tempmail.com", "throwaway.email", "getnada.com"];
    ...
    try {
        ...
        if (messages.length > 0) {
            ...
            if (messageDates.length > 0) {
                ...
                console.log(
                    `  Email date range: ${oldestDate.toISOString()} to ${newestDate.toISOString()} (spanning ${daysSpan} day${daysSpan !== 1 ? "s" : ""})`,
                );
                console.log(
                    `  Oldest email: ${oldestDate.toISOString()} (${Math.round((Date.now() - oldestDate.getTime()) / (1000 * 60 * 60 * 24))} days ago)`,
                );
            }
        }
        ...
        for (let i = 0; i < messages.length; i++) {
            ...
            if (isSentByUser && message.to && message.to.length > 0) {
                for (const to of message.to) {
                    ...
                    if (emailLower && emailLower !== userEmail && !automatedResults.get(emailLower)) {
        ...
        for (let i = 0; i < messages.length; i++) {
                    ...
                    if (emailLower && emailLower !== userEmail && !automatedResults.get(emailLower)) {
                        ...
                        contactsMap.set(parsed.email, {
                            ...
                            first_interaction_at: existing?.first_interaction_at
                                ? new Date(existing.first_interaction_at) < new Date(date)
                                    ? existing.first_interaction_at
                                    : date
                                : date,
                            last_interaction_at: existing?.last_interaction_at
                                ? new Date(existing.last_interaction_at) > new Date(date)
            ...
            if (!isSentByUser && message.from && message.from.length > 0) {
                ...
                if (emailLower && emailLower !== userEmail && !automatedResults.get(emailLower) && contactsWeveEmailed.has(emailLower)) {
                    emailsWithInteractions.add(emailLower);
                    const existing = contactsMap.get(parsed.email);
        ...
        for (const contact of contacts) {
            ...
            const name = contact.first_name && contact.last_name ? `${contact.first_name} ${contact.last_name}` : contact.first_name || contact.email_address;
        ...
        if (messages.length > 0) {
            const messageDates = messages.map((m) => (m.date ? new Date(m.date * 1000) : null)).filter((d): d is Date => d !== null);

            if (messageDates.length > 0) {
                oldestEmailDate = messageDates.reduce((oldest, current) => (current.getTime() < oldest.getTime() ? current : oldest));
                newestEmailDate = messageDates.reduce((newest, current) => (current.getTime() > newest.getTime() ? current : newest));
            }
        }
        ...
        if (oldestEmailDate && newestEmailDate) {
            ...
            console.log(
                `  Email date range: ${oldestEmailDate.toISOString()} to ${newestEmailDate.toISOString()} (${daysSpan} day${daysSpan !== 1 ? "s" : ""} span)`,
            );
        }
...
export async function syncCalendarContacts(grantId: string, userId: string, lastSyncAt?: Date | null) {
    ...
    try {
        ...
        for (const event of events) {
            const eventDate = event.when?.startTime ? new Date(event.when.startTime * 1000).toISOString().split("T")[0] : "no-date";
        ...
        for (const event of events) {
            ...
            const date = event.when?.startTime ? new Date(event.when.startTime * 1000).toISOString() : new Date().toISOString();
            ...
            if (event.participants && event.participants.length > 0) {
                for (const participant of event.participants) {
                    if (
                        ...
                        !userEmail ||
                        participant.email.toLowerCase() === userEmail
                    ) {
                        continue;
                    }
