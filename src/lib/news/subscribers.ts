...
export const DEFAULT_PREFERRED_SEND_TIMES: PreferredSendTime[] = [{ dayOfWeek: 5, hour: 9 }];
...
function mapToSubscriberInterface(dbSubscriber: {
    ...
    return {
        ...
        selectedCounties: dbSubscriber.subscriber_counties.map((c) => c.counties?.name).filter((c): c is string => c !== null && c !== undefined),
