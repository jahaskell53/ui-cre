...
Example: [["Sacramento"], ["Other"], ["San Francisco", "Alameda"]]`;
    ...
    try {
        ...
        const { validatedCategories } = validateCountyCategories(retryCategories, articles, validCounties);
...
export async function getCountyCategories(
    articles: { title: string; description?: string; currentCounties?: string[]; reason?: string }[],
): Promise<string[][]> {
    if (articles.length === 0) return [];
...
${articles
    .map((article, index) => {
        const previousCounties = Array.isArray(article.currentCounties) && article.currentCounties.length > 0 ? article.currentCounties.join(", ") : "None";
...
Example: [["Sacramento"], ["Other"], ["San Francisco", "Alameda"]]`;
        ...
        const { validatedCategories, articlesWithInvalidCounties } = validateCountyCategories(categories, articles, validCounties);
        ...
        if (articlesWithInvalidCounties.length > 0) {
            ...
            const invalidCountyNames = Array.from(new Set(articlesWithInvalidCounties.flatMap((item) => item.invalidCounties)));

            const retryCategories = await retryCountyCategorization(retryArticles, invalidCountyNames, validCounties, validCountiesList);
...
export async function getCityCategories(
    articles: { title: string; description?: string; currentCities?: string[]; currentCounties?: string[]; reason?: string }[],
): Promise<string[][]> {
    if (articles.length === 0) return [];
...
${articles
    .map((article, index) => {
        const previousCities = Array.isArray(article.currentCities) && article.currentCities.length > 0 ? article.currentCities.join(", ") : "None";
        const previousCounties = Array.isArray(article.currentCounties) && article.currentCounties.length > 0 ? article.currentCounties.join(", ") : "None";
        const reason = article.reason || "None";
        return `${index}. Title: ${article.title}
   Description: ${article.description || "No description"}
