...
import { splitArticlesIntoNationalAndLocal, generateEmailContentFromArticles } from './newsletter-utils'
...
function makeFullArticle(overrides: Partial<{
  title: string
  link: string
  description: string
  date: string
...
describe('splitArticlesIntoNationalAndLocal', () => {
  ...
  it('includes all articles as local when no locations are selected', () => {
    const articles = [
      makeArticle({ counties: ['Suffolk'] }),
      makeArticle({ cities: ['Boston'] }),
      makeArticle(),
    ]
...
describe('generateEmailContentFromArticles', () => {
  ...
  it('renders tags as styled spans', () => {
    const result = generateEmailContentFromArticles(
      [makeFullArticle({ tags: ['office', 'industrial'] })],
      []
    )
  ...
  it('renders county geographic tags with County: prefix', () => {
    const result = generateEmailContentFromArticles(
      [makeFullArticle({ counties: ['Providence'] })],
      []
    )
  ...
  it('renders rationale when provided', () => {
    const result = generateEmailContentFromArticles(
      [makeFullArticle({ rationale: 'Highly relevant to your interests.' })],
      []
    )
  ...
  it('renders article title as a link', () => {
    const result = generateEmailContentFromArticles(
      [makeFullArticle({ title: 'Big CRE Deal', link: 'https://example.com/article' })],
      []
    )
