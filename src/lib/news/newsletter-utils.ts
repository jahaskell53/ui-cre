...
export async function fetchArticlesForNewsletter(
    counties?: string[],
    cities?: string[],
    interests?: string,
    geminiModel?: string,
): Promise<NewsletterArticles> {
    try {
...
export function splitArticlesIntoNationalAndLocal<T extends { counties?: string[]; cities?: string[]; tags?: string[]; isNational?: boolean }>(
    ...
    for (const article of articles) {
        ...
        const matchesCounty = selectedCounties && selectedCounties.length > 0 ? selectedCounties.some((county) => articleCounties.includes(county)) : false;

        const matchesCity = selectedCities && selectedCities.length > 0 ? selectedCities.some((city) => articleCities.includes(city)) : false;
...
export function generateEmailContentFromArticles(
    ...
    const generateArticleHtml = (article: {
        ...
        const geographicTagsHtml =
            geographicTags.length > 0
    ...
    if (nationalArticles.length > 0) {
        content +=
            '<div class="section-header" style="font-size: 20px; font-weight: bold; color: #2c3e50; margin: 30px 0 15px 0; padding-bottom: 10px; border-bottom: 2px solid #2c3e50;">National</div>';
    ...
    if (localArticles.length > 0) {
        content +=
            '<div class="section-header" style="font-size: 20px; font-weight: bold; color: #2c3e50; margin: 30px 0 15px 0; padding-bottom: 10px; border-bottom: 2px solid #2c3e50;">Local</div>';
