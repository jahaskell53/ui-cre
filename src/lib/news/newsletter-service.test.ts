...
describe("EmailService.sendEmail", () => {
    ...
    it("includes cc in mail options when provided", async () => {
        ...
        expect(mockSendMail).toHaveBeenCalledWith(expect.objectContaining({ cc: "cc@example.com" }));
    });

    it("omits cc from mail options when not provided", async () => {
        const service = makeService(true);
        await service.sendEmail("to@example.com", content);
        expect(mockSendMail).toHaveBeenCalledWith(expect.not.objectContaining({ cc: expect.anything() }));
    });
...
describe("EmailService.sendNewsletterToSubscriber", () => {
    ...
    it("sends to the subscriber email address", async () => {
        ...
        expect(spy).toHaveBeenCalledWith("alice@example.com", expect.anything(), undefined, expect.any(String));
    });
    ...
    it("forwards cc when provided", async () => {
        ...
        expect(spy).toHaveBeenCalledWith(expect.any(String), expect.anything(), "bcc@example.com", expect.any(String));
    });
    ...
    it("plain text body includes county location when counties are set", async () => {
        const service = makeService();
        const spy = vi.spyOn(service, "sendEmail");
        await service.sendNewsletterToSubscriber(makeSubscriber({ selectedCounties: ["Providence", "Kent"] }), "");
        const [, { text }] = spy.mock.calls[0];
