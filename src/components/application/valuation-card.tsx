...
export function ValuationCard({ title, value, irr, noi, compact }: ValuationCardProps) {
    return (
        <div className={compact ? "rounded-xl bg-black p-5 text-white" : "rounded-xl bg-black p-6 text-white"}>
            <h3 className="mb-2 font-medium text-white">{title}</h3>
            <p className={compact ? "text-3xl font-bold" : "text-4xl font-bold"}>${value.toLocaleString()}</p>
