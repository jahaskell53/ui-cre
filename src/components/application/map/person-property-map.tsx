...
export const PersonPropertyMap = ({ className, addresses, personName }: PersonPropertyMapProps) => {
    ...
    useEffect(() => {
        ...
        geocodedAddresses.forEach((geocoded) => {
            ...
            root.render(<MapPopupContent personName={personName} category={null} label={geocoded.label} address={geocoded.address} />);
      ...
      `;
            ...
            const marker = new mapboxgl.Marker({ element: el, anchor: "bottom" }).setLngLat(geocoded.coordinates).setPopup(popup).addTo(map.current!);
    ...
    return <div ref={mapContainer} className={className} style={{ width: "100%", height: "300px", borderRadius: "8px", overflow: "hidden" }} />;
};
