...
We attach every record to an official **parcel ID (APN)**. Realie already gives us parcel numbers for its records. For Zillow-only or fuzzy cases, we match on the standardized address, and if that fails we use the property’s coordinates to see which parcel boundary it falls inside (spatial join).
