...
- **Unit/integration tests**: `bun run test` (Vitest + jsdom). No external services needed — all deps are mocked.
...
- **Run tests**: `cd pipeline/dagster && .venv/bin/pytest` (all mocked, no external services needed).
