...
If that ordering is awkward for testing, an alternative is to apply the schema change first using **Supabase MCP** (remember: in Cloud Agent VMs MCP targets **production**—see above). That does **not** have to be raw DDL—use whichever MCP-supported operations fit the change. Then still add the corresponding migration file under `supabase/migrations/` from the Drizzle workflow (`drizzle-kit generate`, reviewed and committed). Here **no-op** means: when `supabase db push` runs that migration later, applying its SQL should have **no additional effect** on the database because the target schema already matches what MCP did—the migration mainly advances migration history to stay in sync with `schema.ts`. If the Drizzle-generated statements would error when replayed (for example object already exists), replace or trim them so the file still records the version but runs cleanly. You still edit `src/db/schema.ts` and treat Drizzle as the source of truth.

**Do not apply schema changes via the Supabase MCP or the Supabase dashboard SQL editor as the sole record of schema.** Prefer the Drizzle → migration file → CI path above so changes stay version-controlled and reproducible. The MCP-first workflow in the previous paragraph is allowed when it simplifies pre-merge testing; even then, never skip committing a migration file alongside the schema change.
