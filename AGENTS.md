...
**Statement timeout for large data migrations**: The `supabase db push` migrations role has a **2-minute `statement_timeout`**. Any migration that runs a large backfill (bulk INSERT, UPDATE, or DELETE over tens-of-thousands of rows) will likely exceed this budget and be rolled back. Add `SET LOCAL statement_timeout = 0;` as the **first statement** in any migration that performs bulk DML. This disables the timeout for that transaction only and is safe because the migration is already wrapped in a transaction by `supabase db push`. Example migrations that use this pattern: `20260503120000_crexi_exclude_per_unit_sales_from_trends.sql`, `20260503234944_crexi_api_comps_split_json_tables.sql`.
