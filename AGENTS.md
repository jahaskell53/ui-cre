...
- **ORM convention**: Server-side data access uses `db` from `src/db/index.ts` (Drizzle ORM). The `supabase` server client (`createClient` from `src/utils/supabase/server.ts`) is for auth (`getUser`), realtime, and storage only. Client-side hooks (`use-user.ts`, `use-page-tour.ts`) and client components in `src/app/(app)/` still use the Supabase client directly for now (tracked in OPE-3e).
