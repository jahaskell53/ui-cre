-- Replace "off market" with rental-appropriate market activity metrics:
-- - new_listings: listings first seen in that week
-- - accumulated_listings: listings active during that week (first_seen <= week <= last_seen)
-- - closed_listings: listings that disappear after that week (heuristic; filters out one-week noise)
--
-- Also align REIT toggle with UI semantics:
-- - p_reits_only=false => non-REIT listings only (building_zpid IS NULL)
-- - p_reits_only=true  => REIT listings only (building_zpid IS NOT NULL)
