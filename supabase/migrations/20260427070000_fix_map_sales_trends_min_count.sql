-- Fix pct_change calculation in all map sales trends RPCs.
--
-- Bug: the original functions compared the single oldest month (prior) vs the
-- single newest month (current) in the window. Because commercial sales are
-- sparse, either anchor could be a single outlier transaction that makes the
-- % change wildly wrong (e.g. one $200 K sale in the last month of a 3-year
-- window producing -87% when the actual trend is flat/up).
--
-- Fix: split the window in half and compare the median of ALL transactions in
-- the first half against the median of ALL transactions in the second half.
-- This mirrors what the sales trends chart does in "year" granularity and is
-- robust to outliers because each half pools many transactions.
--
-- Implementation: use FILTER clauses on PERCENTILE_CONT so the spatial JOIN
-- (expensive) runs only once per function, not twice.
-- Regions where either half has < 2 transactions get pct_change = NULL (grey).
