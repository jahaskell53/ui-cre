-- Add LIMIT/OFFSET support to get_zillow_map_listings for sidebar pagination.
--
-- When the map is in cluster mode (low zoom), the client still needs a page
-- of listings for the sidebar. Rather than fetching all rows, it passes
-- p_limit=50 to get only what the sidebar needs.
--
-- We DROP the old function first to avoid the ambiguous-overload problem that
-- PostgREST hits when two functions share the same prefix of parameter types.
