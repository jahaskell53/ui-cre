-- Fix N+1 correlated subquery in reit_buildings unit_mix computation.
--
-- The old query computed unit_mix with a correlated subquery inside
-- reit_buildings:
--
--   (SELECT jsonb_agg(...) FROM (
--     SELECT ... FROM base b2 WHERE b2.building_zpid = base.building_zpid ...
--   ) sub)
--
-- This executes once per REIT building. With 499 buildings each scanning all
-- 6,066 base rows, that is ~3M row reads adding ~540ms of execution time.
--
-- Fix: pre-aggregate unit_mix into a unit_mix_agg CTE over the entire base
-- set once, then join it into reit_buildings. Cost drops from O(B * N) to O(N)
-- where B = number of buildings, N = size of base CTE.
...
-- Pre-aggregate unit_mix per building_zpid in a single pass over base.
-- Replaces the correlated subquery that previously ran once per building (N+1).
unit_mix_agg AS (
    SELECT
        building_zpid,
        COALESCE(
...
reit_buildings AS (
    SELECT
        ...
        -- ARRAY_AGG then [1]: unit_mix_agg has one row per building_zpid, so every
        -- joined row carries the same value. Pick the first element to avoid
        -- GROUP BY on a large jsonb blob (which forces an expensive disk sort).
