-- Optimization 1: dedicated run_id DESC index for fast latest-run lookup.
--
-- The `latest_run` InitPlan does: SELECT run_id FROM cleaned_listings ORDER BY run_id DESC LIMIT 1
-- The existing index is (zip_code, run_id) so Postgres must scan all 120k+ entries to find the
-- global max. A single-column DESC index makes this a single index step (O(1)).
CREATE INDEX IF NOT EXISTS idx_cleaned_listings_run_id_desc
    ON cleaned_listings (run_id DESC);

-- Optimization 2: rewrite get_zillow_map_listings to use a GiST bbox operator.
--
-- The old bbox predicate used ST_X/ST_Y scalar comparisons which are invisible to the GiST index.
-- Replacing with `geom && ST_MakeEnvelope(west, south, east, north, 4326)` lets Postgres do a
-- BitmapAnd of (GiST spatial result) AND (btree run_id result), cutting heap fetches from ~26k to
-- ~2k for a typical city viewport.
...
WITH latest_run AS (
    -- Single index step thanks to idx_cleaned_listings_run_id_desc.
    SELECT run_id
...
base AS (
    ...
    WHERE cl.geom IS NOT NULL
      ...
      -- bbox: use && with ST_MakeEnvelope so the GiST index is used.
      -- Falls back to no filter when all four bounds are NULL.
      AND (
